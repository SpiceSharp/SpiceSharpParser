...
- Source waveform mappings cover `SIN` / `SINE`, `PULSE`, `EXP`, `PWL`, `AM`, `SFFM`, and wave-file input. LTspice finite-cycle `PULSE(... Ncycles)` is supported; finite-cycle `SINE` arguments remain targeted errors, PWL file errors and unsupported repeat syntax produce targeted diagnostics, wave-file channel defaults are not inferred, and topology-changing independent-source instance options remain targeted errors.
...
- Added PWL file fixtures for the supported local header-plus-data file format and targeted diagnostics for missing files, empty files, missing data rows, malformed rows, and unsupported LTspice repeat syntax.
...
- Defer finite-cycle `SINE` behavior, broader LTspice PWL file/repeat variants, and source-option topology synthesis until fixture-backed runtime behavior is specified.
