...
namespace SpiceSharpParser.ModelReaders.Netlist.Spice.Readers.Controls
{
    ...
    public class SaveControl : ExportControl
    {
        ...
        private void AddOpPointToSeries(ParameterSweep firstParameterSweep, Export export, IReadingContext context, Series series)
        {
            ...
            {
                var firstParameterSweepValue = context.SimulationEvaluators.GetSimulationEvaluator(export.Simulation).GetParameterValue(firstParameterSweep.Parameter.Image);
            ...
            foreach (var simulation in context.Result.Simulations)
            {
                ...
                foreach (Entity entity in context.Result.Circuit.Objects)
                {
                    if (entity is SpiceSharp.Components.Component c)
                    {
                        ...
                        context.Result.AddExport(Mapper.Get("i").CreateExport("i", @params, simulation, context.NodeNameGenerator, context.ObjectNameGenerator));
                    }
                }
                ...
                foreach (var node in nodes)
                {
                    ...
                    context.Result.AddExport(Mapper.Get("v").CreateExport("v", @params, simulation, context.NodeNameGenerator, context.ObjectNameGenerator));
                }
            }
        }
        ...
        private void AddLetExport(IReadingContext context, Type simulationType, SingleParameter s)
        {
            ...
            var expressionNames = context.SimulationEvaluators.GetExpressionNames();
