    ...
    public class AMGenerator : WaveformGenerator
    {
        ...
        public override IWaveformDescription Generate(ParameterCollection parameters, ICircuitContext context)
        {
            ...
            if (parameters.Count == 1 && parameters[0] is VectorParameter v)
            {
                ...
                {
                    w.Amplitude = context.Evaluator.EvaluateDouble(v.Elements[0].Value);
                }

                if (v.Elements.Count >= 2)
                {
                    w.Offset = context.Evaluator.EvaluateDouble(v.Elements[1].Value);
                }

                if (v.Elements.Count >= 3)
                {
                    w.ModulationFrequency = context.Evaluator.EvaluateDouble(v.Elements[2].Value);
                }

                if (v.Elements.Count >= 4)
                {
                    w.CarrierFrequency = context.Evaluator.EvaluateDouble(v.Elements[3].Value);
                }

                if (v.Elements.Count >= 5)
                {
                    w.SignalDelay = context.Evaluator.EvaluateDouble(v.Elements[4].Value);
                }
