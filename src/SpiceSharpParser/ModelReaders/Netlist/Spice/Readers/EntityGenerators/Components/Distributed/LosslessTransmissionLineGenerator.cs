        ...
        public override IEntity Generate(string name, string originalName, string type, ParameterCollection parameters, ICircuitContext context)
        {
            ...
            foreach (Parameter parameter in parameters)
            {
                if (parameter is AssignmentParameter ap)
                {
                    ...
                    {
                        context.Result.Validation.Add(new ValidationEntry(ValidationEntrySource.Reader, ValidationEntryLevel.Warning, $"Invalid parameter: {parameter}", parameter.LineInfo));
                    }
                }
            }
